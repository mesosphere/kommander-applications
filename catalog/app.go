...
// Upgrade applies the configured version over a previously installed version.
// When appVersionToInstall is empty, the latest version directory is used.
...
// versionDirs returns all version subdirectories of dir, sorted lexicographically.
// os.ReadDir guarantees lexicographic ordering, so no explicit sort is needed.
// Files and dot-prefixed entries (for example .catalog-source.yaml) are skipped.
...
func applicationDir(application string) (string, error) {
	...
	return filepath.Join(root, "applications", application), nil
}

// catalogRoot returns the catalog checkout that contains applications/.
// NKP_CATALOG_DIR wins when set; otherwise walk up from the working directory.
func catalogRoot() (string, error) {
	...
	for {
		if info, statErr := os.Stat(filepath.Join(dir, "applications")); statErr == nil && info.IsDir() {
			return dir, nil
		}
		parent := filepath.Dir(dir)
		if parent == dir {
			return "", fmt.Errorf("no applications directory found walking up from working directory; set %s", catalogDirEnv)
		}
		dir = parent
	}
}
