...
// catalogRoot returns external catalog checkout root used by environment
// helpers. NKP_CATALOG_DIR points to checkout root containing applications and
// common; it does not change paths owned by nkp-catalog-tests.
...
// absolutePathToBase returns common/base from external catalog checkout.
func absolutePathToBase() (string, error) {
	...
	return filepath.Join(root, "common", "base"), nil
}
...
func (e *Env) ApplyYAML(ctx context.Context, path string, substitutions map[string]string, targets ...ClusterTarget) error {
	...
	err := filepath.Walk(path, func(path string, info os.FileInfo, walkErr error) error {
		...
		if err := applyYAMLFile(ctx, client, path, substitutions, true); err != nil {
...
func (e *Env) ApplyYAMLWithoutSubstitutions(ctx context.Context, path string, targets ...ClusterTarget) error {
	...
	err := filepath.Walk(path, func(path string, info os.FileInfo, walkErr error) error {
		...
		if err := applyYAMLFile(ctx, client, path, nil, false); err != nil {
			return fmt.Errorf("could not apply the YAML file for path: %s :%w", path, err)
		}
