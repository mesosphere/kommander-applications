...
func (v *HelmReleaseValidator) Name() string {
	return "HelmRelease " + v.helmRelease.Name
}

func (v *HelmReleaseValidator) Check(ctx *Context) (done bool, errs []error) {
	defer func() {
		if done {
			ref := metasToRef(v.helmRelease.TypeMeta, v.helmRelease.ObjectMeta)
			ctx.MarkChecked(ref)
		}
	}()

	for _, dep := range v.helmRelease.Spec.DependsOn {
		ref := NewObjectRef(
			v.helmRelease.APIVersion, v.helmRelease.Kind,
			dep.Namespace, dep.Name,
		)
		if ref.Metadata.Namespace == "" {
			ref.Metadata.Namespace = v.helmRelease.Namespace
		}
	...
	ctx.V(1).Infof("Validating Helm release: %q", v.helmRelease.Name)
	rendered, err := renderHelmRelease(ctx, v.helmRelease)
	if err != nil {
		return true, []error{err}
	}
	return true, validateResource(ctx, rendered)
}
...
func addHelmRepo(ctx *Context, helmRepo *fluxsourcesv1beta1.HelmRepository) {
	...
	ctx.SetObject(metasToRef(metav1.TypeMeta{Kind: helmRepo.Kind}, helmRepo.ObjectMeta), helmRepo)
}
...
func renderHelmRelease(ctx *Context, helmRelease *fluxhelmv2beta1.HelmRelease) ([]byte, error) {
	...
	chartValues, err := getChartValues(ctx.Registry, helmRelease)
	if err != nil {
		return nil, err
	}
...
func getChart(ctx *Context, client *action.Install, helmRelease *fluxhelmv2beta1.HelmRelease) (*chart.Chart, error) {
	...
	case "HelmRepository":
		sourceRef := helmRelease.Spec.Chart.Spec.SourceRef
		ref := NewObjectRef(
			sourceRef.APIVersion, sourceRef.Kind,
			sourceRef.Namespace, sourceRef.Name,
		)
		helmRepo, ok := ctx.GetObject(ref).(*fluxsourcesv1beta1.HelmRepository)
		if !ok {
	...
	case "GitRepository":
		...
		return loader.Load(filepath.Join(ctx.RootDir, chart))
	default:
		return nil, fmt.Errorf("Helm source %q not implemented in test yet.", helmRelease.Spec.Chart.Spec.SourceRef.Kind)
	}
}

func getChartValues(registry *Registry, helmRelease *fluxhelmv2beta1.HelmRelease) (map[string]interface{}, error) {
	...
	for _, valuesFrom := range helmRelease.Spec.ValuesFrom {
		...
		case "ConfigMap":
			ref := NewObjectRef(
				"v1", valuesFrom.Kind,
				helmRelease.Namespace, valuesFrom.Name,
			)
			configMap, ok := registry.GetObject(ref).(*v1.ConfigMap)
			if !ok {
